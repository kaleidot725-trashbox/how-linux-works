...
static void child_nf(int id, struct timespec *buf, int nrecord, unsigned long nloop_per_resol, struct timespec start)
{
    int i;
    for (i = 0; i < nrecord; i++) {
        struct timespec ts;
...
static void paranet_fn(int nproc)
{
    int i;
    for (i = 0; i < nproc; i++)
        wait(NULL);
...
int main(int argc, char *argv[]) 
{
    ...
    if (nproc < 1) {
        fprintf(stderr, "<nrpoc>(%d) should be >= 1\n", nproc);
        exit(EXIT_FAILURE);
    }
    ...
    if (total % resol) {
        fprintf(stderr, "<total>(%d) shouild be multiple of <resolution>(%d)\n", total, resol);
        exit(EXIT_FAILURE);
    }
    ...
    puts("estimating workload which takes just one millsecond");
    ...
    for (i = 0, ncreated = 0; i < nproc; i++, ncreated++) {
        ...
        if (pids[i] < 0) {
            ...
            // children
            child_nf(i, logbuf, nrecord, nloop_per_resol, start);
            nice(5);

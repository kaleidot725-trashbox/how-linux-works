...
int main(int argc, char *argv[])
{
    ...
    if (!strcmp(argv[2], "on")) {
        ...
        fprintf(stderr, "kernel's helop should be 'on' or 'off': %s\n", argv[2]);
        exit(EXIT_FAILURE);
    }
    ...
    if (!strcmp(argv[3], "r")) {
        ...
        fprintf(stderr, "kernel's help should be 'r' or 'off': %s\n", argv[3]);
        exit(EXIT_FAILURE);
    }
    ...
    if (offset == NULL) 
        err(EXIT_FAILURE, "mallock() failed");
    ...
    int secotr_size;
    if (ioctl(fd, BLKSSZGET, &secotr_size) == -1) 
        err(EXIT_FAILURE, "ioctl() failed");

    char *buf;
    int e;
    e = posix_memalign((void **)&buf, secotr_size, block_size);
    if (e) {
        errno = e;
